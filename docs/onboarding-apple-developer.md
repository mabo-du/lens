...
> **✋ STOP — v0.2.1 release blocker checklist (read FIRST):**
>
> Before the v0.2.1 release matrix can emit a signed macOS `.dmg`, the
> following seven GitHub-Actions secrets MUST be present on
> `mabo-du/lens` (`Settings → Secrets and variables → Actions`):
>
> | # | Secret | What it is |
> |---|--------|-----------|
> | 1 | `TAURI_SIGNING_PRIVATE_KEY` | generated by `tauri signer generate` |
> | 2 | `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | the passphrase used in step 1 |
> | 3 | `APPLE_ID` | primary email on your Apple Developer Program enrolment |
> | 4 | `APPLE_PASSWORD` | **app-specific** password from Step 5 below (NOT the Apple ID password) |
> | 5 | `APPLE_TEAM_ID` | 10-char alphanumeric from Apple Developer Portal |
> | 6 | `ASC_API_KEY` | base64 of an App Store Connect API key (in-app notarytool alternative to #3 + #4) |
> | 7 | `APPLE_CERTIFICATE` | base64 of the `Developer ID Application` `.p12` from Step 4 |
>
> Until all 7 are configured, the release.yml matrix stalls the macOS
> cell silently — tauri-action waits for `notarytool` to authenticate
> against an Apple ID the runner doesn't have. Confirm with
...
> configured):** the released `.dmg` for v0.2.1 will be UNSIGNED and
> ship with the Gatekeeper workaround `xattr -cr /Applications/LENS.app`
> documented in `docs/USER_GUIDE.md § troubleshooting`. Document the
> unsigned-bundle state explicitly in the v0.2.1 GitHub Release notes
> so users know to expect the workaround. This is consistent with the
> project's rc-history (every rc shipped unsigned until GA).
